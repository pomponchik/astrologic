...
...и мы решили вставить код этой функции (слегка ви) в ```b()```, которая выглядит так:
...
Но тут возникает еще одна проблема. Обратите внимание, обе функции используют одно общее имя - ```print```. Если мы его подменим в одной из них, все сломается. Что отличает имя ```print``` от имени, к примеру, ```result```, которое тоже используется в обеих функциях? Ответ - оно не было порождено в одной из данных функций. Итак, наша задача: подменить только те имена из тех, что создают коллизию, которые были порождены внутри наших функций. Как в функциях появляются новые имена? Если исключить разную эзотерику вроде манипуляций с ```locals()``` или ```globals()```, а также редко встречающиеся в нормальном коде штуки вроде импортов внутри функций, остаются два основных способа: 1. использование имени как параметра в функции; 2. прямое присвоение значения имени с помощью знака ```=```. Что ж, теоретически мы все поняли, приступим к реализации.
