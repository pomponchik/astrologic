...
## Инлайнинг вызова функций

В C++ есть интересная фича - инструкция [```inline```](https://en.wikipedia.org/wiki/Inline_function). Она дает компилятору необязательное к исполнению указание на то, что какую-то функцию можно целиком встроить в другую, из которой она вызывается. Это такая микрооптимизация, позволяющая нам иногда сэкономить пару тактов на том, что в лишний раз не увеличится [стек вызовов функций](https://ru.wikipedia.org/wiki/%D0%A1%D1%82%D0%B5%D0%BA_%D0%B2%D1%8B%D0%B7%D0%BE%D0%B2%D0%BE%D0%B2).

Чтобы сделать что-то подобное на питоне, нужно решить одну важную проблему - конфликт имен. Дело в том, что для компилятора C++ все переменные в конечном счете представляют собой указатели на области в памяти. А у указателей есть одно важное свойство - они всегда уникальные. Две переменные в разных функциях могут называться одинаково, но если мы переведем код этих функций в соответствующие машинные инструкции, там уже не будет этих имен. В python же имена переменных никуда не исчезают даже в процессе выполнения кода. Они доступны, к примеру, через средства интроспекции.
...
```python
def b():
    # Мне очень стыдно, что я не смог придумать примера получше.
    result = 'lol '
    result = 'kek'
    print(result)
    kek = result
...
Как видите, результат не совсем тот, который мы хотели бы получить.

Решить эту проблему довольно просто. Необходимо имена, используемые в функции ```a()```, проверить на уникальность по отношению к именам в функции ```b()```. Если видим коллизию, необходимо подменить каждое из этих имен на какое-то другое, уникальное.

Но тут возникает еще одна проблема. Обратите внимание, обе функции используют одно общее имя - ```print```. Если мы его подменим в одной из них, все сломается. Что отличает имя ```print``` от имени, к примеру, ```result```, которое тоже используется в обеих функциях? Ответ - оно не было порождено в одной из данных функций. Итак, наша задача: подменить только те имена из тех, что создают коллизию, которые были порождены внутри наших функций. Как в функциях появляются новые имена? Если исключить разную эзотерику вроде манипуляций с ```locals()``` или ```globals()```, а также редко встречающиеся в нормальном коде штуки вроде импортов внутри функций, остаются два основных способа: 1. использование имени как параметра в функции; 2. прямое присвоение значения имени с помощью знака ```=```. Что ж, теоретически мы все поняли, приступим к реализации. Для удобства мы продолжим называть встраиваемую функцию именем ```a()```, а ту, куда мы ее имплантируем - именем ```b()```.

Прежде всего, нам необходимо определить все имена, которые использовались в функции ```b()```. Для этого запустим обход AST, в процессе которого все встреченные имена будут складываться в множество ```all_original_names```:
...
ссылка на самомодифицирующийся код: https://ru.wikipedia.org/wiki/%D0%A1%D0%B0%D0%BC%D0%BE%D0%BC%D0%BE%D0%B4%D0%B8%D1%84%D0%B8%D1%86%D0%B8%D1%80%D1%83%D1%8E%D1%89%D0%B8%D0%B9%D1%81%D1%8F_%D0%BA%D0%BE%D0%B4
